...
Public privacy-policy website for the NoAI browser extension.
...
The initial policy follows the extension behavior described in the maintainer-provided conversation: local preferences and checked-video cache, YouTube disclosure checks, and no developer backend or analytics. This repository contains the policy website, not the extension source. Recheck the policy against the extension when its implementation changes.
